...
CHUNK_SIZE = 100  # Data Store hard cap: "Only 200 rows can be updated at once"
...
def handler(context, basicio):
    ...
    try:
        ...
        for local_col, ref_table, ref_col in fk_columns:
            ...
            # Inline pagination logic to pull all records from the parent table
            lookup = {}
            limit = 100
            offset = 1
            
            while True:
                query = f"SELECT ROWID, {ref_col} FROM {ref_table} LIMIT {limit} OFFSET {offset}"
                
                # FIX: Changed from execute_zcql_query to execute_query
                chunk = zcql.execute_query(query) 
                
                if not chunk:
                    break
                    
                for row in chunk:
                    table_data = row.get(ref_table, {})
                    val = str(table_data.get(ref_col, ""))
                    row_id = table_data.get("ROWID")
                    if val and row_id:
                        lookup[val] = row_id
