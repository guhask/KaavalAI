...
def handler(context, basicio):
    ...
    try:
        if action == "start":
        ...
        elif action == "direct_insert_act":
            # Act has 0 rows despite bulk-write reporting "Completed" earlier —
            # same silent-failure pattern seen with State. Only 4 rows, no FK
            # dependencies of its own, so simplest to embed and insert directly.
            rows = [
            ...
            act_table = datastore.table("Act")
            result = act_table.insert_rows(rows)
            basicio.write(json.dumps({"status": "success", "inserted": result}))
